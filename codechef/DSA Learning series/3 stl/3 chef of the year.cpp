...
void printHmp(map<string, string> hmp){
    
    for(auto c : hmp){
        cout << c.first << ":" << c.second <<  newl;     
    }

}

void printHmp(map<string, int> hmp){
...
void solve(){
    ...
    int n, m;   
    cin >> n >> m;       
    map<string, string> hmp_chef;   
    map<string, int>  hmp_countryCount, hmp_individualCount;           
    string str1, str2;   
    forn(i, n){
        cin >> str1 >> str2;       
        hmp_chef[str1] = str2;   
    ...
    forn(i, m){
        cin >> str1;   
        hmp_individualCount[str1]++;    
        hmp_countryCount[hmp_chef[str1]]++;    
    }

    string best_country, best_chef;     
    int bestcountryvote = INT32_MIN, bestchefvote = INT32_MIN;    
    // choose best country
    for(auto c : hmp_countryCount){
        if(c.second > bestcountryvote){
            bestcountryvote = c.second; 
            best_country = c.first;     

        }else if(c.second == bestcountryvote){
            if(c.first < best_country){
                best_country = c.first;     
            }
        }
    }
    ...
    // choose best chef
    for(auto c : hmp_individualCount){
        if(c.second > bestchefvote){
            bestchefvote = c.second;   
            best_chef = c.first;    
        
        }else if(c.second == bestchefvote){
            if(c.first < best_chef){
                best_chef = c.first;   
            }
        }
