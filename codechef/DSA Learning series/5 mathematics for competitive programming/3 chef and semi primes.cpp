...
vi generateSemiPrimes(vi &primes){
...
void solve(){
    ...
    cin >> t;
    vi::iterator it;   
    vi primes = sieveOfEratosthenes(200);     
    vi semiPrimes = generateSemiPrimes(primes);    
    sortall(semiPrimes);    
    ...
    while(t--){
        ...
        for(int i=0; i<semiPrimes.size(); i++){
            ...
            oe = n - semiPrimes[i];
            it = lower_bound(semiPrimes.begin(), semiPrimes.end(), oe);   
            if(oe == *it){
                canBeFormed = true;    
                break; 
            }    
