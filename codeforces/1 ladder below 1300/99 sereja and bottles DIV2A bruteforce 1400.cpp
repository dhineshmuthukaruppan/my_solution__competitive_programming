...
int main(){
    ...
    int answer = 0;  
    for(int i=0; i<n; i++){
        // if a[i][0] present in mp1 atleast one time or greater than 1 time in mp2 then the bottle can be opened else increment the count
        if(mp1[a[i][0]] > 0 || mp2[a[i][0]].first > 1){
            continue;      
        }else if(mp2[a[i][0]].first == 1 && mp2[a[i][0]].second != i){
            continue;        
        }else{
            answer++;   
        }         
