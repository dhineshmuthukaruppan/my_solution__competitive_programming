...
void solve(){
    ...
    while(t--){    
        ...
        // resulting sequence should be nonincresing after inserting "b into a" randomly
        cin >> n >> k;    
        vi a(n), b(k);    
        condition = false;  
        x = -1;       
        ...
        forn(i, n){
            ...
            if(x == -1 && a[i] != 0){
                ...
                // condition = true means non increasing can be formed
                if(a[i]!= 0 && x > a[i]){
                    condition = true;
                    break;      
                }else if(a[i]!=0 && x < a[i]){
                    x = a[i];        
                }else if(a[i] == 0 && k==1 && x > b[0] && x!=-1){
                    condition = true;
                    break;     
                }else if(a[i] == 0 && k==1  && (x==-1 || (x!=-1 && x<b[0]))){  
                    x = b[0];           
                }     
            }
        }
