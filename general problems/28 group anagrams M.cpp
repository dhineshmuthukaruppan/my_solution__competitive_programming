...
vector<vector<string>> solve(vector<string> &vec){
    ...
    unordered_map<string, vi> hmap;    

    for(int i=0; i<vec.size(); i++){
        string sorted_string = stringSort(vec[i]);    
        hmap[sorted_string].push_back(i);    
    }
...
int main(){
    ...
    vector<string> vec {"yo", "act", "flop", "tac", "cat", "oy", "olfp"};   
    vector<vector<string>> result = solve(vec);    
    cout << result << '\n';        
