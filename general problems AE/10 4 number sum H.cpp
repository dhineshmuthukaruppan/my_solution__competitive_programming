...
vvi solve(vi &a, int target){
    ...
    for(int i=0; i<n-1; i++){
        for(int j=i; j<n; j++){
            int currentPairSum = a[i] + a[j];   
            int requiredNumberOnhmp = target - currentPairSum;    
            auto it = hmp.find(requiredNumberOnhmp); 
            if(it != hmp.end()){
                // found on hashmap
                for(int k=0; k<(it->second).size(); k++){
                    result.pb({(it->second)[k].first, (it->second)[k].second, a[i], a[j]});        
                }    
            }
        }  /* end of for j loop */

        for(int j=0; j<i; j++){
            int sumToBeAddedToHashMap = a[i] + a[j];   
            auto it = hmp.find(sumToBeAddedToHashMap);   
            if(it != hmp.end()){
                // element found on the hashmap
                (it->second).pb({a[j], a[i]});     
            }else{
                // element not found on the hashmap
                hmp[sumToBeAddedToHashMap] = {{a[j], a[i]}};    
            }
        }
