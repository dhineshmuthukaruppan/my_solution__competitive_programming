...
int solve(){
    ...
    while(!found){
        ...
        palindrome = makePalindrome(to_string(firstHalf));     

        for(int i=999; i>99; i--){

            if(palindrome / i > 1000 || i < sqrt(palindrome)){
                break;  
            }

            if(palindrome % i == 0){
                first_factor = i;  
                second_factor = palindrome/i; 
                found = true;   
                break; 
            }
    ...
    cout << first_factor << " " << second_factor << " " << palindrome << '\n';     
